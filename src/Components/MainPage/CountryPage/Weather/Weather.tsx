...
const Weather = ({ lang, capital }: IWeather) => {
  const [{ id, descrition, temp }, setWeather] = React.useState<IWeatherData>({
    id: '',
    descrition: '',
    temp: 0
  });
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          {id ? (
            <i
              style={{
                fontSize: '100px',
                color: `${+id.substring(4) >= 800 ? '#FFA570' : '#bde0fe'}`
              }}
              className={`${css.weatherIcon} ${css.owf} ${css[id]}`}
            />
          ) : (
            <Spin style={{ width: '100px' }} size="large" />
          )}
        </div>
      )}
    >
